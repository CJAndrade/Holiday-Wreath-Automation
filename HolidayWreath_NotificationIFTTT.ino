...
int tonePin = 5; //connect the piezo buzzer to pin#5
String readString;
bool firstTime = true;
int pirPin = 2;               //connect the PIR sensor to pin#2

//Your Wifi router setup at home
char ssid[] = "XXXXXXXX";   //your network SSID (aka WiFi name)
char pass[] = "XXXXXXXXXXXXX"; //your network password
int status = WL_IDLE_STATUS;
WiFiServer server(80);
//Follow the project on Hackster.io to setup the IFTTT
...
void loop() {
  if (digitalRead(pirPin) == HIGH)
   {
    Serial.println("Motion detected"); // aka someone is at the door
    ifttt_motion_detected(); //send notification aka a email or sms via an ifttt recipe
    rainbowCycle(10);//light up the NeoPixels on wreath in a nice rainbow
    joyToWorldMidi();//play the midi if motion is detected, keep the folks at the door entertained.
  }
        ...
        if (readString.length() < 100) {
         ...
         if (c == '\n') {
           ...
           if (readString.indexOf("?red") >0){
            ...
            //joyToWorldMidi(); //commented, we will play midi only if the PIR sensor detects someone
            delay(10);
           }
...
void ifttt_motion_detected() {
  String motion_detected = "Y";
  String  data = "{\"value1\":\"" + motion_detected + "\"}";
...
void joyToWorldMidi() {
  //the Credit for this piece of code goes to - https://www.extramaster.net/tools/midiToArduino/
